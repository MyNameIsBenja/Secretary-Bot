    ...
    if (commandName === 'bl') {
        ...
        collector.on('collect', async (buttonInteraction) => {
            ...
            if (buttonInteraction.customId === 'approved') {
                try {
                    ...
                    await buttonInteraction.update({ embeds: [confirmationEmbed] });
                } catch (error) {
                    console.error(`Failed to ban ${username}:`, error);
                    await buttonInteraction.reply({ content: "-# Failed to ban user.", ephemeral: true });
                }
                ...
                await buttonInteraction.update({ embeds: [deniedEmbed] });
            }
        });

        collector.on('end', collected => {
            if (collected.size === 0) msg.edit({ components: [] });
        });
    } else if (commandName === 'blacklist-database') {
        if (bansDatabase.length === 0) {
            return interaction.reply('-# No users in the blacklist.');
        }
        ...
        bansDatabase.forEach(ban => {
            embed.addFields(
                ...
                { name: 'Ban Date', value: ban.banDate, inline: true } // Show ban date
            );
        });
        ...
        const index = bansDatabase.findIndex(ban => ban.userId === userId);
        if (index === -1) {
            return interaction.reply("-# User not found in the blacklist.");
        }

        bansDatabase.splice(index, 1); // Remove user from the blacklist
        await interaction.reply(`-# User with ID ${userId} removed from the blacklist.`);
    }
});
